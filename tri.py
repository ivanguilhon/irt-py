...
def W(theta, alpha=1, beta=5, r=0.2):
    ...
    return (
        Pp(theta, alpha, beta, r)
        * Qp(theta, alpha, beta, r)
        / (P(theta, alpha, beta, r) * Q(theta, alpha, beta, r))
    )
...
# classe questao
class questao:
    def __init__(self, N: int, acertos: np.ndarray) -> None:
        self.N = N
        self.J = acertos.shape[0]
        self.respostas = acertos[N]
        self.acertos = self.calcacertos()
    ...
    def calcHabilidade(self, pontos: np.ndarray) -> np.ndarray:
    ...
    def estimateParams(self, acertos, nSteps=100):
        ...
        habilidadeSummary = []
        acertoSummary = []
        habilidade = acertos.sort_values("Pontos padronizados", ascending=True)[
            "Pontos padronizados"
        ]
        ...
        self.padraoAcertoExp = [
            np.asarray(habilidadeSummary),
            np.asarray(acertoSummary),
        ]
    ...
    def calcParams(self):
        def custo(xi):
            ...
            for j in range(self.J):
                ca += (
                    (1.0 - self.r)
                    * (self.acertos[j] - self.P(self.habilidade[j]))
                    * self.W(self.habilidade[j])
                    * (self.habilidade[j] - self.beta)
                )
                cb += 0.0 - self.alpha * (1.0 - self.r) * (
                    self.acertos[j] - self.P(self.habilidade[j])
                ) * self.W(self.habilidade[j])
                cr += (
                    (self.acertos[j] - self.P(self.habilidade[j]))
                    * self.W(self.habilidade[j])
                    / self.Pp(self.habilidade[j])
                )
            return [ca, cb, cr]
    ...
    def calcBeta(self):
        def custoBeta(beta):
            ...
            for j in range(self.J):
                # print (j)
                custo += 0.0 - self.alpha * (1.0 - self.r) * (
                    self.acertos[j] - self.P(self.habilidade[j])
                ) * self.W(self.habilidade[j])
            return custo
    ...
    def verossemelhanca(self, param):
        ...
        self.setR(param[2])
        soma = self.acertos * np.log(self.P(self.habilidade)) + (
            1 - self.acertos
        ) * np.log(self.Q(self.habilidade))
        return -soma.sum()

    def logL(self):
        soma = 0
        for j in range(self.J):
            soma += self.acertos[j] * np.log(self.P(self.habilidade[j])) + (
                1 - self.acertos[j]
            ) * np.log(self.Q(self.habilidade[j]))
        return soma
    ...
    def jacobian(self, param):
        ...
        for j in range(self.J):
            ...
            fator = (self.acertos[j] - self.P(self.habilidade[j])) / (
                self.P(self.habilidade[j]) * self.Q(self.habilidade[j])
            )
            somaA += (
                -fator
                * (1 - self.r)
                * (theta - self.beta)
                * np.exp(-self.alpha * (theta - self.beta))
                / (1 + np.exp(-self.alpha * (theta - self.beta))) ** 2
            )
            somaB += (
                -fator
                * (1 - self.r)
    ...
    def optParameters(self, method="Powell"):
        if method == "Powell":
            ...
            res = opt.minimize(
                self.verossemelhanca,
                x0,
                method="Powell",
                bounds=((0, 1.5), (-5, 5), (0, 1.0)),
                tol=1e-4,
            )
            # print (res)
        ...
        if method == "BFGS":
            ...
            res = opt.minimize(
                self.verossemelhanca, x0, jac=self.jacobian, method="BFGS", tol=1e-4
            )
            # print (res)
            self.setAlpha(res.x[0])
            self.setBeta(res.x[1])
    ...
    def makeFigure(
        ...
        if showExp:
            plt.plot(
                self.padraoAcertoExp[0] * stdHab + meanHab,
                self.padraoAcertoExp[1],
                linewidth=0,
                marker="o",
                alpha=0.1,
                color="blue",
        ...
        if refCurve:
            ...
            plt.plot(
                x,
                100 * I(x, r=0.15, alpha=0.1, beta=65),
                linestyle="dashed",
                color="red",
            )
...
class questaoAberta(questao):
    ...
    def classificarRespostas(self):
        ...
        self.acertos["isC"] = (self.acertos[questaoIndex] > corteC) & (
            self.acertos[questaoIndex] < corteB
        )
        self.acertos["isB"] = (self.acertos[questaoIndex] > corteB) & (
            self.acertos[questaoIndex] < corteA
        )
        self.acertos["isA"] = self.acertos[questaoIndex] > corteA
        self.acertos[["isD", "isC", "isB", "isA"]] = self.acertos[
            ["isD", "isC", "isB", "isA"]
    ...
    def PAberta(self, theta):
        ...
        return [
            expC / (1 + expC),
            1.0 / (1 + expC) - 1.0 / (1 + expB),
            1.0 / (1 + expB) - 1.0 / (1 + expA),
            1.0 / (1 + expA),
        ]

    def logL(self):
        self.acertos["logLD"] = (self.acertos["isD"]) * np.log(
            self.PAberta(self.habilidade)[0]
        ) + (1.0 - self.acertos["isD"]) * np.log(1 - self.PAberta(self.habilidade)[0])
        self.acertos["logLC"] = (self.acertos["isC"]) * np.log(
            self.PAberta(self.habilidade)[1]
        ) + (1.0 - self.acertos["isC"]) * np.log(1 - self.PAberta(self.habilidade)[1])
        self.acertos["logLB"] = (self.acertos["isB"]) * np.log(
            self.PAberta(self.habilidade)[2]
        ) + (1.0 - self.acertos["isB"]) * np.log(1 - self.PAberta(self.habilidade)[2])
        self.acertos["logLA"] = (self.acertos["isA"]) * np.log(
            self.PAberta(self.habilidade)[3]
        ) + (1.0 - self.acertos["isA"]) * np.log(1 - self.PAberta(self.habilidade)[3])
        # soma += self.acertos[j]*np.log( self.P(self.habilidade[j]) ) + (1- self.acertos[j]) * np.log( self.Q(self.habilidade[j]) )
        return (
            self.acertos["logLD"].sum()
            + self.acertos["logLC"].sum()
            + self.acertos["logLB"].sum()
            + self.acertos["logLA"].sum()
        )
    ...
    def optParameters(self):
        ...
        res = opt.minimize(
            self.minusVerossemelhanca,
            x0,
            method="Powell",
            bounds=((0, 1.5), (-5.0, 5.0), (-0.0, 5.0), (-0.0, 5.0)),
            tol=1e-4,
        )
        # print (res)
        self.setAlpha(res.x[0])
...
class Aluno:
    ...
    def logL(self, theta):
        self.parametros["nula"] = (self.parametros["tAcerto"] > 0.999) | (
            (self.parametros["questao"] >= 25) & (self.parametros["questao"] <= 36)
        )
        self.parametros["nula"].replace({True: 1.0, False: 0.0})

        self.parametros["P"] = self.parametros["rs"] + (
            1.0 - self.parametros["rs"]
        ) * 1.0 / (
            1
            + np.exp(-self.parametros["alphas0"] * (theta - self.parametros["betas0"]))
        )
        self.parametros["Q"] = 1 - self.parametros["P"]
        self.parametros["acerto_ij"] = self.acertos
        self.parametros["logL_ij"] = (1 - self.parametros["nula"]) * (
            (self.parametros["acerto_ij"]) * np.log(self.parametros["P"])
            + (1 - self.parametros["acerto_ij"]) * np.log(self.parametros["Q"])
        )
    ...
    #         for i in range(I):
    #             #print(i)
    #             alpha= self.parametros['alphas0'][i]
    #             beta=  self.parametros['betas0'][i]
    #             r= self.parametros['rs'][i]
    #             logL+=(self.data[i+1])*P(theta,alpha,beta,r)+(1-self.data[i+1])*Q(theta,alpha,beta,r)
    #         return float(logL)
...
def NewtonRaphOpt(questao):
    ...
    def h_ji(questao, theta):
        return np.array(
            [
                (1.0 - questao.r) * (theta - questao.beta),
                -questao.alpha * (1 - questao.r),
                1.0 / questao.Pp(theta),
            ]
        )

    def H_ji(questao, theta):
        H11 = (
            (1 - questao.r) * (1 - 2 * questao.Pp(theta)) * (theta - questao.beta) ** 2
        )
        H12 = -(1 - questao.r) * (
            1 + questao.alpha * (theta - questao.beta) * (1 - 2 * questao.Pp(theta))
        )
    ...
    for j in range(questao.J):
        h += (
            (questao.acertos[j] - questao.P(habilidade[j]))
            * questao.W(habilidade[j])
            * h_ji(questao, habilidade[j])
        )
    h = np.asmatrix(h)
    ...
    for j in range(questao.J):
        ...
        # scorinf-fischer
        H += (
            -questao.Pp(habilidade[j])
            * questao.Qp(habilidade[j])
            * questao.W(habilidade[j])
            * hji.transpose()
            * hji
        )
